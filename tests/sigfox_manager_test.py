...
class TestSigfoxManager:
    ...
    def test_get_contracts(self, mock_get):
        ...
        mock_get.return_value = MagicMock(status_code=200, text='{"data": [{"id": "1", "name": "Contract 1"}]}')
    ...
    def test_get_devices_by_contract(self, mock_get):
        # Mock the API response
        mock_get.return_value = MagicMock(status_code=200, text='{"data": [{"id": "1", "name": "Device 1"}]}')
